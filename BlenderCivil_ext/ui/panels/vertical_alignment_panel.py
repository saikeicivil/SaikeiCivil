...
class VIEW3D_PT_bc_vertical_alignment(Panel):
    ...
    def draw(self, context):
        ...
        # Header with alignment name
        box = layout.box()
        box.label(text=f"Alignment: {vertical.name}", icon='IPO_LINEAR')
        
        # Status indicator
        row = box.row()
        if vertical.is_valid:
            row.label(text="Valid", icon='CHECKMARK')
        else:
            row.label(text="Check Issues", icon='ERROR')
        row.label(text=f"{len(vertical.pvis)} PVIs")
        
        # Validation message
        if vertical.validation_message:
            col = box.column()
            col.scale_y = 0.7
            col.label(text=vertical.validation_message)
        
        # Statistics
        if len(vertical.pvis) >= 2:
            box = layout.box()
            col = box.column(align=True)
            col.label(text="Profile Statistics:", icon='INFO')
            col.separator(factor=0.5)
            
            row = col.row(align=True)
            row.label(text=f"Length: {vertical.total_length:.1f}m")
            row.label(text=f"ΔElev: {vertical.elevation_max - vertical.elevation_min:.1f}m")
